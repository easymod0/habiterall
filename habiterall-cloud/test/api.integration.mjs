 ...
 *
 * The request is deliberately PAGED BACK (`end: isoDaysAgo(PAGE_BACK_DAYS)`),
 ...
 * "within window" check would pass either way. Mirrors habiterall-personal's
 * test/overview.integration.mjs; the two editions promise the same API.
 */
const runsHabit = await (await fetch(`${overviewBase}/api/habits`, {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify(
    { name: 'RunsHabit 247', type: 'boolean', freq_numerator: 3, freq_denominator: 7 }
  ),
})).json();
const RUNS_DAYS = 60;
for (let i = RUNS_DAYS - 1; i >= 0; i--) {
  ...
  if ([1, 3, 5].includes(dow)) {
    await putNoteEntry(runsHabit.id, date, { value: 2 });
  }
}

const GRID_DAYS = 14;
const PAGE_BACK_DAYS = 20;
const runsView = await getOverview({ days: GRID_DAYS, end: isoDaysAgo(PAGE_BACK_DAYS) });
const runsRow = runsView.habits.find((h) => h.id === runsHabit.id);

ck('cloud: runs is a non-empty array',
  Array.isArray(runsRow.runs) && runsRow.runs.length > 0, JSON.stringify(runsRow.runs));
ck("cloud: every run's start/end lies within the requested grid window",
...
ck('cloud: a run\'s length is the TRUE, unclipped length — greater than the days requested',
...
// The row figures are computed in archived mode too, so `runs` must ride
// along there as well.
await fetch(`${overviewBase}/api/habits/${runsHabit.id}`, {
  method: 'PUT',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    name: 'RunsHabit 247', type: 'boolean', freq_numerator: 3, freq_denominator: 7,
...
const runsArchivedView = await getOverview({ days: GRID_DAYS, archived: 'true' });
const runsArchivedRow = runsArchivedView.habits.find((h) => h.id === runsHabit.id);
ck('cloud: ?archived=true still carries the runs field',
...
for (const id of [
  ...
  marathon.id,
  retired.id,
  // #247's fixture: ~26 entries over its 60-day run, same trap as the rest
  // of this list.
  runsHabit.id,
]) {
  await fetch(`${overviewBase}/api/habits/${id}`, { method: 'DELETE' });
