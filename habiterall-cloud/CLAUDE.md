...
are different: they are session-level, and `pg` puts them in the STARTUP
PACKET. Against PgBouncer 1.25.2 in `pool_mode = transaction`, the default
config **refuses the connection** and both `ignore_startup_parameters` and
`track_extra_parameters` connect with **both values zeroed** — there is no
PgBouncer setting that carries them through, because it can only track a
parameter Postgres REPORTS and Postgres reports neither.

**The remedy is to put the values where the pooler is not in the path**, on the
role or the database, which every backend it opens then inherits:
...
answers `0` — the one surface an operator would check is the one that lies.
Nothing exercises any of this: neither edition ships a pooler and CI runs none,
so this paragraph and `docs/decisions/caching.md`'s measured table are the
whole of the guard (#201). Re-measure rather than trusting the table if the
PgBouncer version moved.
...
## Two write paths must take their locks in ONE order

Cloud's mutating paths all reach `users` last through the `data_version` bump, and
...
**The dump used to run inside a multi-replica `app`, and that race is now
removed by DEPLOYMENT SHAPE, not by a lock.** Since #194 the tick, the gateway
and this dump all run in the singleton `notifier` container, so the
no-two-runs-at-once guarantee — `inFlight` and `lastAttemptDate`, both
per-process module state — is sufficient on its own: there is exactly one
process left for it to be true of. It still cannot coordinate a SECOND process
by construction, which is precisely why "run exactly one `notifier`, ever" is a
rule and not a suggestion.

**A crashed run's own `.tmp` is reclaimed at the START of the next run, and
only because of an age gate.** Each run's temporary file carries a random
per-run suffix, which stopped two writers interleaving into one corrupt file —
but it also made a crashed run's leftover un-namable by anything ever again,
invisible to `prunableBackups` and `todaysFileExists`, so a dump-sized file per
crash sat on the volume forever. `reclaimStaleTmp` deletes one only once it is
older than `BACKUP_TIMEOUT_MS + KILL_GRACE_MS` — this module's own upper bound
on how long a LIVE run can hold its `.tmp` open — so by construction nothing
that old belongs to a run in progress, and reclaiming it cannot race a live
writer the way a blanket "delete any `.tmp`" would.

Two more that are not visible from a diff. **`backupConfig` is deliberately
free of logging side effects**: `GET /backup/status` reaches it on every
request, so a version that logged `backup.admin_url_missing` while computing
`enabled` let any one of N tenants drive error-level lines into the operator's
log. `reportBackupConfig` is the separate function that logs, called once, at
boot, from `server.js`, never from a route. And **a write-stream failure kills
the child**: without it `pg_dump` blocks on its own `write()` once the OS pipe
fills, and nothing notices until `BACKUP_TIMEOUT_MS` (two hours) — for those
two hours the child holds its `REPEATABLE READ` snapshot open, pinning the xmin
horizon, while `inFlight` blocks every later attempt.

`docs/decisions/cloud-backups.md` has the long form of all four.
...
file again. With the token gone it exits 0 having done nothing, which keeps
`up` working after the production checklist has you delete it.

Five rules that bite from the app's side:
...
- **The client id and secret are pushed, not read back.** They are generated
  ...
  the IdP are configured from the same two lines.
- **A `CHANGE_ME` value from `.env.example` is refused** for the three that are
  worth something to a stranger holding a public repository: the OIDC pair, and
  `AUTHENTIK_BOOTSTRAP_TOKEN`, which Authentik turns into a full admin API
  token for `akadmin` on every boot.
- **`grant_types` must be sent explicitly.** The field defaults to an empty
  list and an empty list permits nothing: a provider created without it looks
  correct in the admin UI and rejects every sign-in, arriving at the user as a
  500 on `/auth/callback`.
- **Signing out needs TWO redirect URIs and an ID token, and neither half works
  alone** — so the bootstrap and `server.js` change together, and one test
  asserts both. Registering only the callback leaves Authentik's
  `post_logout_redirect_uris` empty and the app's `post_logout_redirect_uri` is
  discarded in silence; registering the logout URI *on its own* is worse, since
  Authentik then validates `id_token_hint` before planning the invalidation
  flow. `completeLogin` therefore returns the ID token beside the user and the
  callback stores it on the session **inside `regenerate`**, which discards
  whatever the old session held. Nothing reads a claim out of it.
- **`pickLogoutFlow` asks what a flow DOES, reading its bindings**, because the
  name is exactly what got this wrong: `default-provider-invalidation-flow`
  looks like the provider's and has no stages at all, so every sign-out ended
  habiterall's session and left Authentik's. Every wrong version still ends
  with the app on its sign-in screen — only asking the provider for a password
...
`PUBLIC_URL` is not HTTPS. **Never set that flag in a real deployment.** The
OIDC issuer string must also resolve identically from the browser *and* the app
container, or token validation fails on an issuer mismatch; locally, compose
aliases the host via `extra_hosts`.

`docs/decisions/auth.md` has the rest — the blueprint context rules and the
three things about them that were found by running them, what a signed-out user
sees and where each string comes from, why brand settings leak into Authentik's
admin UI, and the trap that Authentik's request log does not record the
invalidation flow.
