...
so every write path that can move what those two figures mean calls
`clearHabitSummary` or `clearAllSummaries` (`db.js`) itself: both branches of
the entry PUT, the entry DELETE, `PUT /habits/:id` (it REPLACES, so `type` and
`target_*` can move under the cached pair), the three sites in
`apply-import.js`, **the three writers of `atMostUnlogged`** — `PUT /settings`,
`DELETE /settings` and `POST /import`'s settings restore — since that setting
is an INPUT to `recomputeBestStreak` and so clears every habit's stamp rather
than one, and **`record()` in `notifier.js`**, the shared handler behind every
ntfy and Discord button press, which writes outside the `/api` router entirely.
...
including a theme toggle that cannot move any figure; narrowing it to the keys
that are inputs is a rule to revisit every time a setting becomes one, so the
blanket clear is the safe direction rather than the free one.

**A write path added later that skips this serves a figure from before the
write for the REST OF THAT DAY** — not forever, because `summary_asof` holds
the day it was computed for and `summaryCacheHit` is an equality test. That
bound is worth knowing in both directions: it is why a missed call is a wrong
number rather than a permanently wrong one, and why it is easy to not notice,
since it heals overnight and never reproduces for whoever comes to look.

There is no CHECK constraint holding "a stamp with no figures behind it is
never served" — SQLite cannot add a table-level CHECK with `ALTER TABLE`, so
that invariant lives only in `summaryCacheHit`
(`@habiterall/shared/summary-cache.js`), the one reader both editions call. And
the write-back carries no `data_version` guard the way cloud's does, because
`DatabaseSync` is synchronous: the entry reads, the derivation and the stamp
are one uninterrupted turn of the event loop. **That holds only while the route
stays synchronous between the derivation and the stamp** — an `await` inserted
there reopens exactly the race cloud's guard exists for.
`docs/decisions/caching.md` has the design in full.
