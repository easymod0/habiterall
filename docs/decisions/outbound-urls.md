...
instance — one bucket for every tenant on it — so one account's sleep used to be
paid by everybody behind it in a sequential loop.
...
entirely: its limit is per webhook, so parallelising across accounts is free.
A single global limit would have been wrong for one of the two whichever number
was picked. See `docs/decisions/reminders.md` for the concurrency figures.
