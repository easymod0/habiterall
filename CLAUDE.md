...
| `habiterall-cloud/` | `habiterall-cloud/CLAUDE.md` | `auth.md`, `connectivity.md`, `caching.md`, `sharing-and-rls.md`, `cloud-backups.md` |
...
| `shared/test/browser/` | `shared/test/browser/CLAUDE.md` | `testing.md` |
...
miss. Say so before "fixing" it. Under `success` the same mechanism adds CREDIT
instead, and there it needs evidence: an unanswered day counts as success only
from the habit's first row that STATES a value, so a skip cannot buy compliance
...
**Two write paths in one edition must take their locks in ONE order, and a
statement added to a shared write wrapper is a lock-order question first.**
This is cloud's in practice — `habiterall-cloud/CLAUDE.md` has the rule, the
five pairs of routes that deadlocked, and the matrix for when a statement may
skip a lock instead of ordering it. Nothing in either edition handles `40P01`,
so the victim is a 500 on somebody's tap.
...
**`shared/test/browser/CLAUDE.md` is where the rest lives** — what each suite
exists for, the join rules for a reload or a navigation, the template-literal
trap, and why the worker count is what it is. Read it before adding a suite or
changing how one waits.
